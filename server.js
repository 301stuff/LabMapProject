...
function Event(query) {
  ...
  this.summary = query.description.text;
}
...
function searchEvents(){
  const url = `https://www.eventbriteapi.com/v3/events/search?location.address=${targetLocation.name}&location.within=10km&expand=venue&token=${process.env.EVENTBRITE_API_KEY}`;
  return superagent.get(url)
    .then(res => {
      let events = res.body.events.map((event) => new Event(event));
      return events;
    });
}
